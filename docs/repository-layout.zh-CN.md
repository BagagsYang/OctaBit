...
该仓库是 MIDI-8bit Synthesiser 产品族的单体仓库。它包含公开的 Flask Web 应用、原生
macOS 和 Windows 应用、规范 Python 渲染器、共享预览资源、部署文件和发布文档。
...
| `.github/workflows/` | Windows 发布构建的 GitHub Actions 工作流。 |
| `compose.web.yml` | Flask Web 部署的 Docker Compose 入口。 |
| `global.json` | Windows 解决方案使用的 .NET SDK 选择。 |
...
原生 WinUI 3 Windows 应用、C# 渲染器、测试、安装程序和评审工具。
...
Windows 应用有自己的 C# 渲染器，并在对齐测试中用 Python 参考渲染器进行校验。应用工程会从规范
`assets/previews/` 目录链接预览 WAV 文件，用于构建和发布输出。
`src/Midi8BitSynthesiser.App/Assets/Previews/` 下也存在一份字节相同的已跟踪副本，但当前工程文件使用共享资源目录作为构建来源。
...
渲染器接收平台无关的文件路径和波形层设置，然后将 WAV 文件写入磁盘。Web 和 macOS
会直接调用它；Windows 将它作为原生 C# 渲染器的对齐参考。

### `assets/previews/`

Web、macOS 和 Windows 应用路径共用的规范预览 WAV 资源。`assets/README.md`
记录了它们的预期用途和来源说明。
...
Windows 开发使用 .NET 8 和 Python 渲染器依赖：
...
macOS 通过 Xcode 使用 `MIDI8BitSynthesiser` scheme 构建。Xcode 构建阶段会运行
