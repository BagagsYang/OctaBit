...
该仓库是为 MIDI-8bit Synthesiser 产品族重新整理后的单体仓库。当前活跃目标是 `apps/web-flask/` 中的 Flask/Gunicorn Web 服务，并通过 Docker 面向服务器部署。原生 macOS 和 Windows 应用已 deprecated/paused，不再作为活跃开发目标；代码保留在仓库中，用于参考或未来可能的恢复。Python 参考渲染器位于 `core/` 下，共享预览资源位于 `assets/` 下。
